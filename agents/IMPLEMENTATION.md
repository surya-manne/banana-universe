...
- **`apps/example-rest-postgresql`** — DDD-style catalog module, TypeORM (PostgreSQL locally, **sql.js** in tests), **tsyringe** providers + repository **InjectionToken**, bearer auth, pagination, optional OTel, Swagger; `docker-compose.yml`, integration tests
...
- **`docs-site/recipes/index.md`** — recipes index + nav **Recipes**; **`.github/workflows/ci.yml`** — typecheck steps for packages + example apps (including `example-fastify`)
