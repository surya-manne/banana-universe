...
- **`apps/example-rest-postgresql`** — **`main.ts`** / **`bootstrap.ts`** (lowercase entry); **`src/modules/catalog/`** with **`createModule`** (controller not duplicated in **`providers`**), TypeORM, domain port **`CatalogItemMapper.ts`** + **`CatalogItemMapperToken`**, list/query Zod in **`CatalogDto.ts`** (no separate list-query file); bearer auth, pagination, optional OTel, Swagger; integration tests
- **`apps/example-rest-mongodb`** — **`mongoose.connect`** + default connection; **`ArticleModel.ts`** registers on default connection (no connection passed into **`getArticleModel()`**); **`main.ts`** / **`bootstrap.ts`**; PascalCase module files; health test with dummy **`DATABASE_URL`**
...
- **`bananajs-cli` presets** (`create-app-presets.ts`) — MongoDB preset matches **`example-rest-mongodb`** (**`main.ts`**, **`ArticlesModule`**, no **`@injectable()`** on controller); SQL preset **`HealthModule`** with **`providers: []`**; **`generate-module` / `generate-ai-module`**: domain **`Mapper`** port + PascalCase generated filenames (not entry files)
...
| Date       | Change                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 2026-03-29 | **Recipes + presets DX**: all **`apps/example-*`** use **`src/modules/<feature>/`** + **`createModule`**; shared **`eslint.app.config.mjs`**; **`main.ts`** / **`bootstrap.ts`** (lowercase); **PascalCase** feature modules only; **`createModule`** does not list **`controller`** in **`providers`**; controllers omit **`@injectable()`** where not needed; domain port files **`Mapper.ts`** (CLI generators + PostgreSQL catalog); MongoDB uses **`mongoose.connect`** + model on default connection; **`docs-site`** recipes + layered-architecture updated |
