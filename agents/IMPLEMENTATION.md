...
- **plugin-zod**: deprecated re-export shim (decommissioned and removed on 2026-04-22); **`plugin-websocket`**: `@WsBody(zodSchema?)`
...
| Date       | Change                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-04-29 | **docs-site beta branding refresh**: home page hero and supporting copy updated to a stronger gold-on-dark visual treatment; added public beta messaging to the home page, browser/site title, footer, and persistent nav title badge via VitePress theme CSS and config                                                                                                                                                                                                              |
| 2026-04-22 | **plugin-zod decommissioned**: removed `packages/plugin-zod`; updated `tsconfig.json`, local publish script, CI workflow, docs-site plugin overview, zod integration docs, and project documentation (`CONTEXT`, `ARCHITECTURE`, `DEPENDENCIES`, `TECHSTACK`, `CODEMAP`); regenerated `package-lock.json` to remove workspace lock entries; release dry-run confirms package is no longer targeted. |
| 2026-03-29 | **`bananajs-cli`**: **`bjs g`** interactive (type + name); **`generate module`** matches **`new`** layout (**`src/modules/...`**, **`createModule`**, bootstrap + TypeORM **`entities[]`** patch, **`--skip-bootstrap`**); **`ai generate`** TTY flow (DDD vs flat); removed **`ai wizard`**; interactive **routes** / **db** / **openapi export** / **migrate**; README + docs-site CLI/AI updates                                                                                  |
| 2026-03-29 | **AIRoadmapV1 execution**: shared **`bananajs-ai-rules`**, **`.bananarc` `project`** context, structured **`ai review`** (+ SARIF, **`--module`**), **`ai wizard`**, **`ai wire`**, **`ai test`**, **`ai explain`**, **`ai doc`** deprecation notice; **`apps/example-rest-dual-orm`** (README, **`.env.example`**, **`MONGODB_URI`** vs **`DATABASE_URL`**); **docs-site `/ai/`** hub + nav; **CI** dual-ORM typecheck + Mongo smoke test; **`docs/ARCHITECTURE.md`** bananarc note |
